...
47. [complete] 继续深化半年度 resolution Module：把人生愿望、行动预览、长期系统投影、成就/里程碑、回合上下文刷新与终局判定集中到半年度权威记录 seam；同时压缩行动决策 Dock，避免控件换行挤占展示区。

48. [complete] 架构候选 2：拆分 session 持久化与状态发布，新增 `state_publication.py` 作为 gameplay/live 发布 Adapter seam，让 `state_manager.save_session()` 回归纯 durable persistence。

49. [complete] 架构候选 4：新增 AI enrichment Adapter，把 prompt 读取、OpenAI JSON 调用、AI 输出归一和 no-AI Adapter 从 `game_logic.py` 中拆出，保持确定性 gameplay artifact 权威。

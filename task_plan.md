...
47. [complete] ??????? resolution Module?????????????????????/?????????????????????????? seam????????? Dock?????????????

48. [complete] ???? 2??? session ??????????? `state_publication.py` ?? gameplay/live ?? Adapter seam?? `state_manager.save_session()` ??? durable persistence?

49. [complete] ???? 4??? AI enrichment Adapter?? prompt ???OpenAI JSON ???AI ????? no-AI Adapter ? `game_logic.py` ????????? gameplay artifact ???

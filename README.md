...
- 模块化事件池：阶段事件已拆分到独立模块，并为不同年龄阶段和行动类型提供结构化事件、标签、伏笔与轻量状态偏向。
- 连续选择反馈：连续多个半年投入同一主行动会形成“连续投入”判定修正，同时记录对应的状态惯性和机会成本提醒。
- 行动预览：正式模拟页会在提交前展示当前行动的愿望同频、预计 D100 目标值、主/辅属性、风险属性和连续投入预判，帮助玩家做更清晰的取舍。
...
- 布局优化：人生模拟阶段的底部行动栏有桌面/移动端高度上限，内部滚动且输入栏保持可见，避免遮挡主叙事展示区。
...
backend/app/
  ...
  event_pool.py        模块化阶段事件池、事件标签、伏笔与状态偏向
  fate_mapper.py       命理参数到游戏状态、判定修正与年度变化
  game_logic.py        一命千途阶段流转、行动预览、连续选择反馈、可选 AI 命盘/GM 增强与半年模拟
